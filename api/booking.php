...
if(isset($_POST['pre_book'])) {
    ...
    if($query_token) {
        ...
        $mail->SMTPAuth = true;
        $mail->Username = 'shaikram022@gmail.com'; // Your gmail
        $mail->Password = 'owjemtmbikkxaeoy'; //Your gmail app password
        $mail->SMTPSecure = 'ssl';
        $mail->Port = 465;

        $mail->setFrom('shaikram022@gmail.com'); //Your gmail
    ...
    // $database->where("token", $token);
    // $tokenExist = $database->getOne(TOKEN_TABLE);
    // $count = count($tokenExist);
    ...
    // $database->insert(TOKEN_TABLE, $token_values);
    // $token_query = $database->insert(TOKEN_TABLE, $token_values);
    ...
    //     
    //     $preBook_query = $database->insert(TOKEN_TABLE, $preBook_values);
        
    //     if($preBook_query && $mail->send()) {
